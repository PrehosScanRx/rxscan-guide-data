...
if (!args["rollback-reason"]?.trim()) throw new Error("--rollback-reason is required");
...
const next = nextChannelPointer({
  ...
  rollbackReason: args["rollback-reason"],
  rollback: true,
...
if (args["dry-run"]) console.log("Dry run: Stable was not modified.");
  ...
  console.log(`Stable rolled back to ${releaseId}.`);
}
